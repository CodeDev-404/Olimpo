...
>
    <aside class="sidebar w-60 bg-ink-950 text-ink-300 flex flex-col h-full shrink-0 fixed lg:static z-40 -translate-x-full lg:translate-x-0 transition-transform duration-200">
        ...
        @foreach($navItems as $item)
            <a href="{{ route($item['route']) }}" wire:navigate
                class="nav-link {{ $activePanel === $item['key'] ? 'nav-link-active' : 'nav-link-inactive' }}">
                {!! $item['svg'] !!}
    ...
    <div @click="toggleSidebar()" class="sidebar-overlay fixed inset-0 bg-black/60 z-30 hidden lg:hidden"></div>
</div>
