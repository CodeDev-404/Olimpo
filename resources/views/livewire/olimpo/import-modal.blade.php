    ...
    <div class="modal-overlay"
        ...
        <div class="modal-card max-w-4xl"
            ...
            <div class="modal-header">
                <div>
                    <h3 class="text-base font-semibold text-ink-900">Importar {{ match($panel) { 'ocurrencias' => 'Ocurrencias', 'personal' => 'Personal', 'asistencia' => 'Asistencia', 'cumpleanos' => 'Cumpleaños', default => $panel } }}</h3>
                    <p class="text-[11px] text-ink-400 mt-0.5">Formatos aceptados: .xlsx, .xls, .csv</p>
                </div>
                <button wire:click="close" class="p-1 text-ink-400 hover:text-ink-700 rounded-md hover:bg-ink-100 transition-colors">
            ...
            <div class="modal-body">
                @if(empty($rows) && !$importing)
                    <div class="border-2 border-dashed border-ink-200 rounded-xl p-10 text-center hover:border-ink-400 transition-colors cursor-pointer"
                        ...
                        <p class="text-sm text-ink-500 font-medium">Haz clic para seleccionar archivo</p>
                ...
                @elseif($importing)
                    <div class="text-center py-10">
                        <svg class="w-8 h-8 mx-auto mb-3 text-ink-500 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                        </svg>
                        <p class="text-sm text-ink-500">Procesando archivo...</p>
                    </div>
                @else
                    ...
                    <div class="overflow-x-auto border border-ink-200 rounded-lg max-h-64 overflow-y-auto">
                                ...
                                @foreach($rows as $i => $row)
                                    <tr class="{{ !$row['valid'] ? 'bg-red-50' : '' }}">
                                        <td>
                                            @if($row['valid'])
                                                <input type="checkbox" wire:model.live="selectedRows" value="{{ $i }}"
                                                    class="rounded border-ink-300 text-ink-800">
                                            @endif
                                        </td>
                                        ...
                                        @if($panel === 'ocurrencias')
                                            ...
                                            <td class="text-ink-500 text-xs max-w-[150px] truncate">{{ $row['preview']['detalles'] }}</td>
                                        @elseif($panel === 'personal')
                                            <td class="font-medium">{{ $row['preview']['nombre'] }}</td>
                                        ...
                                        @else
                                            ...
                                            <td>
                                                <span class="text-xs font-semibold {{ $row['preview']['turno'] === 'NOCHE' ? 'text-ink-500' : ($row['preview']['turno'] === '24H' ? 'text-purple-600' : 'text-blue-600') }}">
                                                    {{ $row['preview']['turno'] }}
                                                </span>
                                            </td>
