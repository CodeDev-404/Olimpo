    ...
    <div class="card mb-5" style="overflow:visible">
        <div class="card-body">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex flex-wrap gap-1 bg-[#f4f6f9] dark:bg-white/[0.06] rounded-lg p-0.5 w-fit" role="tablist">
                ...
                {{-- ===== Toolbar: Control BMA-828 ===== --}}
                <div x-show="tab === 'bma828'" class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                    ...
                    <div x-show="filtersOpen" class="flex items-center gap-3 flex-1 min-w-0 filter-slide-in">
                        ...
                        <button x-show="$wire.search || $wire.filterFecha" @click="$wire.limpiarFiltros()"
                ...
                <div x-show="tab === 'bma828'" class="flex flex-wrap gap-2 ml-auto">
                ...
                {{-- ===== Toolbar: Combustibles ===== --}}
                <div x-show="tab === 'combustibles'" class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                    <div class="relative w-full sm:w-[12.8rem] sm:min-w-[12.8rem]">
                        <i data-lucide="search" class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-ink-400"></i>
                    ...
                    <div x-show="filtersOpenCombustible" class="flex items-center gap-3 flex-1 min-w-0 filter-slide-in">
                        ...
                        <button x-show="$wire.searchCombustible || $wire.filterFechaCombustible" @click="$wire.limpiarFiltrosCombustible()"
                            class="flex items-center gap-1.5 h-9 px-3 rounded-lg text-xs font-medium text-ink-500 dark:text-ink-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 border border-[#e5eaef] dark:border-white/[0.06] hover:border-red-300 dark:hover:border-red-700 transition-all shrink-0">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
                            Limpiar filtros
                        </button>
                ...
                <div x-show="tab === 'combustibles'" class="flex flex-wrap gap-2 ml-auto">
                    <button wire:click="nuevoCombustible" class="btn btn-primary btn-sm">
                        <i data-lucide="plus" class="w-4 h-4 mr-1.5"></i>
                        Nuevo
                    </button>
                    @if($selectModeCombustible && count($selectedIdsCombustible) > 0)
                        <button wire:click="eliminarSeleccionadosCombustible" wire:confirm="¿Eliminar los registros de combustible seleccionados?" class="btn btn-danger btn-sm">
                            Eliminar selección ({{ count($selectedIdsCombustible) }})
                        </button>
                    @endif
                    <button wire:click="toggleSelectModeCombustible" class="btn btn-sm {{ $selectModeCombustible ? 'btn-warning' : 'btn-outline border-ink-200 dark:border-ink-600' }}">
                        {{ $selectModeCombustible ? 'Cancelar' : 'Seleccionar' }}
                    </button>
                ...
                <div class="card-header">
                    ...
                    <span class="text-xs text-ink-500 dark:text-ink-400 font-medium flex items-center gap-1">
                        @if($search)
                            ...
                            <span class="text-ink-300">({{ $totalRegistros }})</span>
                        @else
                            Control de Vehículos
                            <span class="text-ink-300">({{ $totalRegistros }})</span>
                        @endif
                        <span class="text-ink-300 mx-0.5">·</span>
                        <span class="text-ink-400">En ruta {{ $enRuta }}</span>
                        <span class="text-ink-300 mx-0.5">·</span>
                        <span class="text-ink-400">Retornados {{ $retornados }}</span>
                        <span class="text-ink-300 mx-0.5">·</span>
                        <span class="text-ink-600 dark:text-ink-300 font-semibold">Total {{ $totalRegistros }}</span>
                    </span>
                </div>
                                ...
                                <tr>
                                    ...
                                    <th class="w-10">
                                        <input type="checkbox" wire:click="toggleSelectAll"
                                            wire:key="select-all-{{ count($selectedIds) > 0 && !array_diff($registros->pluck('id')->toArray(), $selectedIds) ? 'on' : 'off' }}"
                                    ...
                                    <th class="hidden lg:table-cell">Modelo</th>
                                    <th class="hidden lg:table-cell">Clase</th>
                                    <th class="hidden md:table-cell">H. Salida</th>
                                    <th class="hidden md:table-cell">Km. Salida</th>
                                    <th class="hidden md:table-cell">H. Ingreso</th>
                                    <th class="hidden md:table-cell">Km. Ingreso</th>
                                    <th class="hidden xl:table-cell">Observación</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($registros as $i => $r)
                                    <tr wire:click="selectRegistro({{ $r['id'] }})"
                                        ...
                                        class="cursor-pointer {{ $selectMode && in_array($r['id'], $selectedIds) ? 'bg-ink-50 dark:bg-ink-800' : ($selectedId === $r['id'] && !$selectMode ? 'bg-ink-50 dark:bg-ink-800' : ($i % 2 === 1 ? 'table-row-zebra' : '')) }}">
                                        @if($selectMode)
                                        ...
                                        @endif
                                        <td class="hidden sm:table-cell font-mono text-ink-400 text-xs">{{ $registros->firstItem() + $i }}</td>
                                        ...
                                        <td class="hidden md:table-cell">{!! h($r['marca'] ?? null, $search) !!}</td>
                                        <td class="hidden lg:table-cell">{!! h($r['modelo'] ?? null, $search) !!}</td>
                                        <td class="hidden lg:table-cell">{!! h($r['clase'] ?? null, $search) !!}</td>
                                        <td class="hidden md:table-cell font-mono">{!! h($r['hora_salida'] ?? null) !!}</td>
                                        <td class="hidden md:table-cell font-mono">{!! h($r['km_salida'] ?? null) !!}</td>
                                        <td class="hidden md:table-cell font-mono">{!! h($r['hora_ingreso'] ?? null) !!}</td>
                                        <td class="hidden md:table-cell font-mono">{!! h($r['km_ingreso'] ?? null) !!}</td>
                                        <td class="hidden xl:table-cell max-w-[200px] truncate">{!! h($r['observacion'] ?? null, $search) !!}</td>
                                    </tr>
                                @empty
                                    <tr>
        ...
        <div x-show="tab === 'combustibles'">
            <div class="card">
                <div class="card-header">
                    <div class="flex items-center gap-1.5">
                    <span class="text-xs text-ink-500 dark:text-ink-400 font-medium flex items-center gap-1">
                        @if($searchCombustible)
                            Resultados para '{{ $searchCombustible }}'
                        ...
                        @endif
                        <span class="text-ink-300 mx-0.5">·</span>
                        <span class="text-ink-400">Galones {{ number_format($totalGalones, 1) }}</span>
                        <span class="text-ink-300 mx-0.5">·</span>
                        <span class="text-ink-400">S/ {{ number_format($totalMonto, 2) }}</span>
                        <span class="text-ink-300 mx-0.5">·</span>
                        <span class="text-ink-600 dark:text-ink-300 font-semibold">Total {{ $registrosCombustibles->total() }}</span>
                    </span>
                </div>
                                ...
                                <tr>
                                    ...
                                    <th class="w-10">
                                        <input type="checkbox" wire:click="toggleSelectAllCombustible"
                                            wire:key="select-all-c-{{ count($selectedIdsCombustible) > 0 && !array_diff($registrosCombustibles->pluck('id')->toArray(), $selectedIdsCombustible) ? 'on' : 'off' }}"
                                            {{ count($selectedIdsCombustible) > 0 && !array_diff($registrosCombustibles->pluck('id')->toArray(), $selectedIdsCombustible) ? 'checked' : '' }}>
                                    </th>
                                    @endif
                                    <th class="hidden sm:table-cell">#</th>
                                    <th>Fecha</th>
                                    <th class="hidden lg:table-cell">Cat.</th>
                                    <th class="hidden lg:table-cell">Clase</th>
                                    <th class="hidden md:table-cell">Marca</th>
                                    <th>Placa</th>
                                    <th class="hidden lg:table-cell">Modelo</th>
                                    <th class="hidden xl:table-cell">Año</th>
                                    <th class="hidden xl:table-cell">Color</th>
                                    <th>Conductor</th>
                                    <th class="hidden lg:table-cell">Kilometraje</th>
                                    <th>Combustible</th>
                                    <th>Galones</th>
                                    <th class="hidden md:table-cell">Precio x Galón</th>
                                    <th>Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($registrosCombustibles as $i => $r)
                                    <tr wire:click="selectCombustible({{ $r['id'] }})"
                                        ...
                                        class="cursor-pointer {{ $selectModeCombustible && in_array($r['id'], $selectedIdsCombustible) ? 'bg-ink-50 dark:bg-ink-800' : ($selectedCombustibleId === $r['id'] && !$selectModeCombustible ? 'bg-ink-50 dark:bg-ink-800' : ($i % 2 === 1 ? 'table-row-zebra' : '')) }}">
                                        @if($selectModeCombustible)
                                        <td class="w-10" wire:click.stop="toggleSelectCombustible({{ $r['id'] }})">
                                            <input type="checkbox" wire:key="row-c-{{ $r['id'] }}-{{ in_array($r['id'], $selectedIdsCombustible) ? 'on' : 'off' }}"
                                                {{ in_array($r['id'], $selectedIdsCombustible) ? 'checked' : '' }}>
                                        </td>
                                        @endif
                                        <td class="hidden sm:table-cell font-mono text-ink-400 text-xs">{{ $registrosCombustibles->firstItem() + $i }}</td>
                                        <td class="font-medium">{!! h($r['fecha'] ?? null, $searchCombustible) !!}</td>
                                        <td class="hidden lg:table-cell">{!! h($r['categoria'] ?? null, $searchCombustible) !!}</td>
                                        <td class="hidden lg:table-cell">{!! h($r['clase'] ?? null, $searchCombustible) !!}</td>
                                        <td class="hidden md:table-cell">{!! h($r['marca'] ?? null, $searchCombustible) !!}</td>
                                        <td class="font-mono font-medium text-ink-900 dark:text-ink-100">{!! h($r['placa'] ?? null, $searchCombustible) !!}</td>
                                        <td class="hidden lg:table-cell">{!! h($r['modelo'] ?? null, $searchCombustible) !!}</td>
                                        <td class="hidden xl:table-cell">{!! h($r['anio'] ?? null) !!}</td>
                                        <td class="hidden xl:table-cell">{!! h($r['color'] ?? null) !!}</td>
                                        <td class="font-medium text-ink-900 dark:text-ink-100 capitalize">{!! h($r['conductor'] ?? null, $searchCombustible) !!}</td>
                                        <td class="hidden lg:table-cell font-mono">{!! h($r['kilometraje'] ?? null) !!}</td>
                                        ...
                                        <td class="hidden md:table-cell text-right font-mono">{{ $r['precio_galon'] ? 'S/ ' . number_format($r['precio_galon'], 2) : '<span class="text-ink-300">—</span>' }}</td>
